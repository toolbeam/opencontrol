...
export const DialogSelect = createDialog({
  ...
  schema: z.object({
    ...
    onSelect: z.function(z.tuple([z.any()])).returns(z.void()).optional(),
    options: z.array(
      z.object({
        display: z.string(),
  ...
  render: (ctx) => {
    ...
    const filtered = createMemo(() => ctx.input.options?.filter(i => i.display.toLowerCase().includes(filter().toLowerCase())))
    ...
    const handleSelection = (index: number) => {
      ...
      // Otherwise, if there's a global onSelect handler, call it with the option's value
        ...
        ctx.input.onSelect(option.value !== undefined ? option.value : option.display);
      }
    };
